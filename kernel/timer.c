...
void timer_init(void){
	...
	*(volatile unsigned long*)(CLINT_BASE + 0x4000 + ((unsigned long)(get_runqueue()->hart_id) * 8)) = curr_time + time_lapse;
	_set_ssie();
	return;
}
...
void update_timer(unsigned long mhartid){
	unsigned long time_lapse = timebase_freq / 100;
	unsigned long curr_time = *(volatile unsigned long*)(CLINT_BASE + 0xBFF8);
	*(volatile unsigned long*)(CLINT_BASE + 0x4000 + (mhartid * 8)) = curr_time + time_lapse;
	return;	
}
